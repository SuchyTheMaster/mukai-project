...
- Akcje `Od nowa` i `Zapisz` znajdują się obok siebie w lewej kolumnie bezpośrednio pod brandingiem. `Zapisz` tworzy ZIP pełnego projektu, jest aktywne po poprawnej inspekcji audio i używa niebieskiego wariantu `secondary`, takiego jak `Wróć do audio`.
