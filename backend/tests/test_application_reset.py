...
class ApplicationResetTest(unittest.TestCase):
    def test_reset_removes_job_and_upload_draft(self):
        ...
        with patch.object(routes.repository, "delete_job") as delete_job, patch.object(routes, "purge_tree") as purge_tree:
            response = routes.reset_application(request)

        self.assertTrue(response.reset)
        delete_job.assert_called_once_with(request.jobId)
        self.assertEqual(
            purge_tree.call_args_list,
            [
                call(f"jobs/{request.jobId}"),
                call(f"drafts/{request.uploadDraftId}"),
            ],
        )
